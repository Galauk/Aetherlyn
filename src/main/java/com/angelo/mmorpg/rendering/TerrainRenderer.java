...
public class TerrainRenderer {
    ...
    private static final float UV_MARGIN = 0.5f / SHEET_W; // anti-bleeding
    ...
    private void buildMesh(WorldMap worldMap) {
        ...
        for (int x = 0; x < w; x++) {
            for (int z = 0; z < h; z++) {
                ...
                // Posição no mundo — tiles ficam de (0,0) a (w,h)
                // O player começa em (w/2, h/2) que é o centro do mapa
                float x0 = x;
                float x1 = x + 1.0f;
                float z0 = z;
                float z1 = z + 1.0f;

                vertices.put(x0).put(0f).put(z0).put(u0).put(v0);
                vertices.put(x1).put(0f).put(z0).put(u1).put(v0);
                vertices.put(x1).put(0f).put(z1).put(u1).put(v1);
                vertices.put(x0).put(0f).put(z1).put(u0).put(v1);
